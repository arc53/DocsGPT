...
def _format_agent_output(agent: dict, *, pinned: bool = False, include_key_masked: bool = True) -> dict:
    ...
    out = {
        ...
        "guardrails_enabled": bool(agent.get("guardrails_enabled", False)),
        "input_guardrails": bool(agent.get("input_guardrails", False)),
        "output_guardrails": bool(agent.get("output_guardrails", False)),
    }
...
def _build_create_kwargs(data: dict, *, image_url: str, agent_type: str) -> dict:
    ...
    for key in (
        "limited_token_mode", "limited_request_mode", "allow_system_prompt_override",
...
class CreateAgent(Resource):
    create_agent_model = api.model(
        "CreateAgentModel",
        {
            ...
            "sources": fields.List(fields.String, required=False, description="List of source identifiers for multiple sources"),
            ...
            "agent_type": fields.String(required=False, description="Type of the agent (classic, react, workflow)."),
            ...
            "json_schema": fields.Raw(required=False, description="JSON schema for enforcing structured output format"),
            "limited_token_mode": fields.Boolean(required=False, description="Whether the agent is in limited token mode"),
            "token_limit": fields.Integer(required=False, description="Token limit for the agent in limited mode"),
            "limited_request_mode": fields.Boolean(required=False, description="Whether the agent is in limited request mode"),
            "request_limit": fields.Integer(required=False, description="Request limit for the agent in limited mode"),
            "models": fields.List(fields.String, required=False, description="List of available model IDs for this agent"),
            ...
            "allow_system_prompt_override": fields.Boolean(required=False, description="Allow API callers to override the system prompt via the v1 endpoint"),
            "guardrails_enabled": fields.Boolean(required=False, description="Enable safety guardrails for the agent"),
            "input_guardrails": fields.Boolean(required=False, description="Enable input guardrails to filter user input before processing"),
            "output_guardrails": fields.Boolean(required=False, description="Enable output guardrails to filter agent responses before delivery"),
        },
    )
    ...
    def post(self):
        ...
        if "json_schema" in data:
            try:
                ...
                return make_response(jsonify({"success": False, "message": "Invalid JSON schema"}), 400)

        if data.get("status") not in ["draft", "published"]:
            return make_response(jsonify({"success": False, "message": "Status must be either 'draft' or 'published'"}), 400)
        ...
        if data.get("status") == "published":
            ...
            if (
                ...
                return make_response(jsonify({"success": False, "message": "Either 'source' or 'sources' field is required for published agents"}), 400)
        else:
        ...
        try:
            ...
            with db_session() as conn:
                ...
                if agent_type == "workflow":
                    pg_workflow_id, err = _resolve_workflow_for_user(conn, data.get("workflow"), user)
                    if err and is_published:
                        return err
                    if pg_workflow_id is None and is_published:
                        return make_response(jsonify({"success": False, "message": "Workflow is required"}), 400)
                ...
                kwargs = _build_create_kwargs(build_data, image_url=image_url, agent_type=agent_type)
                agent_row = AgentsRepository(conn).create(user, data["name"], data["status"], **kwargs)
                ...
                for field in json_fields:
                    if field in data and data[field]:
                        try:
                            ...
                            return make_response(jsonify({"success": False, "message": f"Invalid JSON format for field: {field}"}), 400)
        ...
        try:
            with db_session() as conn:
                ...
                if not existing_agent:
                    return make_response(jsonify({"success": False, "message": "Agent not found or not authorized"}), 404)
                pg_agent_id = str(existing_agent["id"])
                image_url, image_error = handle_image_upload(request, existing_agent.get("image", "") or "", user, storage)
                if image_error:
                    return image_error
                ...
                for field in allowed_fields:
                    ...
                    if field == "status":
                        ...
                        if new_status not in ["draft", "published"]:
                            return make_response(jsonify({"success": False, "message": "Invalid status value. Must be 'draft' or 'published'"}), 400)
                        ...
                        if not source_id or source_id == "default":
                            ...
                            return make_response(jsonify({"success": False, "message": f"Invalid source ID format: {source_id}"}), 400)
                    elif field == "sources":
                        ...
                        for src in sources_list:
                            ...
                            if looks_like_uuid(src):
                                ...
                                return make_response(jsonify({"success": False, "message": f"Invalid source ID in list: {src}"}), 400)
                        ...
                        if chunks_value in ("", None):
                            ...
                            try:
                                ...
                                if chunks_int < 0:
                                    return make_response(jsonify({"success": False, "message": "Chunks value must be a non-negative integer"}), 400)
                                update_fields["chunks"] = chunks_int
                            except (ValueError, TypeError):
                                return make_response(jsonify({"success": False, "message": f"Invalid chunks value: {chunks_value}"}), 400)
                    elif field == "tools":
                        tools_list = data.get("tools", [])
                        if not isinstance(tools_list, list):
                            return make_response(jsonify({"success": False, "message": "Tools must be a list"}), 400)
                        update_fields["tools"] = tools_list
                    elif field == "json_schema":
                        json_schema = data.get("json_schema")
                        if json_schema is not None:
                            try:
                                update_fields["json_schema"] = normalize_json_schema_payload(json_schema)
                            except JsonSchemaValidationError:
                                return make_response(jsonify({"success": False, "message": "Invalid JSON schema"}), 400)
                        else:
                        ...
                        if bool_value and data.get("token_limit") is None:
                            return make_response(jsonify({"success": False, "message": "Token limit must be provided when limited token mode is enabled"}), 400)
                        ...
                        if bool_value and data.get("request_limit") is None:
                            return make_response(jsonify({"success": False, "message": "Request limit must be provided when limited request mode is enabled"}), 400)
                        ...
                        if update_fields["token_limit"] > 0 and not data.get("limited_token_mode"):
                            return make_response(jsonify({"success": False, "message": "Token limit cannot be set when limited token mode is disabled"}), 400)
                        ...
                        if update_fields["request_limit"] > 0 and not data.get("limited_request_mode"):
                            return make_response(jsonify({"success": False, "message": "Request limit cannot be set when limited request mode is disabled"}), 400)
                    elif field == "folder_id":
                        folder_input = data.get("folder_id")
                        if folder_input:
                            pg_folder_id, folder_err = _resolve_folder_id(conn, folder_input, user)
                            if folder_err:
                                return folder_err
                        ...
                        if not normalized:
                            ...
                            pg_workflow_id, wf_err = _resolve_workflow_for_user(conn, workflow_input, user)
                            if wf_err:
                                return wf_err
                        ...
                        if not value or value == "default":
                            ...
                            return make_response(jsonify({"success": False, "message": f"Invalid prompt_id: {value}"}), 400)
                        ...
                        update_fields["allow_system_prompt_override"] = (
                            raw_value == "True" if isinstance(raw_value, str) else bool(raw_value)
                        )
                    elif field in ("guardrails_enabled", "input_guardrails", "output_guardrails"):
                        raw_value = data.get(field, False)
                        update_fields[field] = (
                            raw_value == "True" if isinstance(raw_value, str) else bool(raw_value)
                        )
                    else:
                        ...
                        if field in ["name", "description", "agent_type"]:
                            if not value or not str(value).strip():
                                return make_response(jsonify({"success": False, "message": f"Field '{field}' cannot be empty"}), 400)
                        update_fields[field] = value

                if image_url:
                    update_fields["image"] = image_url
                if not update_fields:
                    return make_response(jsonify({"success": False, "message": "No valid update data provided"}), 400)

                newly_generated_key = None
                final_status = update_fields.get("status", existing_agent.get("status"))
                final_agent_type = update_fields.get("agent_type", existing_agent.get("agent_type"))
                ...
                if final_status == "published":
                    if final_agent_type == "workflow":
                        ...
                        workflow_final = update_fields.get("workflow_id", existing_agent.get("workflow_id"))
                        if not workflow_final:
                            missing_published_fields.append("Workflow")
                        if missing_published_fields:
                            return make_response(jsonify({"success": False, "message": f"Cannot publish workflow agent. Missing required fields: {', '.join(missing_published_fields)}"}), 400)
                    else:
                        missing_published_fields = []
                        ...
                        source_final = update_fields.get("source_id", existing_agent.get("source_id"))
                        extra_final = update_fields.get("extra_source_ids", existing_agent.get("extra_source_ids") or [])
                        retriever_final = update_fields.get("retriever", existing_agent.get("retriever"))
                        if not source_final and not extra_final and not retriever_final:
                            missing_published_fields.append("Source or retriever")
                        if missing_published_fields:
                            return make_response(jsonify({"success": False, "message": f"Cannot publish agent. Missing or invalid required fields: {', '.join(missing_published_fields)}"}), 400)
                ...
                if not updated:
                    return make_response(jsonify({"success": False, "message": "Agent not found or update failed"}), 404)

        except Exception as err:
            current_app.logger.error(f"Error updating agent {agent_id}: {err}", exc_info=True)
            return make_response(jsonify({"success": False, "message": "Database error during update"}), 500)

        response_data = {"success": True, "id": pg_agent_id, "message": "Agent updated successfully"}
            ...
            with db_session() as conn:
                ...
                if agent.get("agent_type") == "workflow" and workflow_id:
                    try:
                        ...
                        current_app.logger.warning(f"Workflow cleanup failed for agent {pg_agent_id}: {wf_err}")
        ...
        try:
            with db_session() as conn:
                ...
                if uuid_pinned:
                    ...
                    result = conn.execute(
                        _sql_text("SELECT * FROM agents WHERE id = ANY(CAST(:ids AS uuid[]))"),
                        {"ids": uuid_pinned},
                    )
            ...
            for agent in pinned_agents:
                ...
                list_pinned_agents.append({
                    ...
                    "updated_at": agent.get("updated_at", ""),
                    "last_used_at": agent.get("last_used_at", ""),
                    "key": (f"{agent['key'][:4]}...{agent['key'][-4:]}" if agent.get("key") else ""),
                    "pinned": True,
                })
        except Exception as err:
...
class GetTemplateAgents(Resource):
    ...
    def get(self):
        try:
            ...
            with db_readonly() as conn:
                result = conn.execute(
                    _sql_text("SELECT * FROM agents WHERE user_id IN ('system', '__system__') ORDER BY name"),
                )
                template_rows = [dict(row._mapping) for row in result.fetchall()]
            template_agents = [
                {"id": str(agent["id"]), "name": agent.get("name"), "description": agent.get("description") or "", "image": agent.get("image") or ""}
            ...
            with db_session() as conn:
                if looks_like_uuid(agent_id):
                    template_row = conn.execute(
                        _sql_text("SELECT * FROM agents WHERE id = CAST(:id AS uuid) AND user_id IN ('system', '__system__')"),
                        {"id": agent_id},
                    ).fetchone()
                else:
                    template_row = conn.execute(
                        _sql_text("SELECT * FROM agents WHERE legacy_mongo_id = :id AND user_id IN ('system', '__system__')"),
                        {"id": agent_id},
                    ).fetchone()
                ...
                create_kwargs: dict = {}
                for col in ("description", "agent_type", "image", "retriever", "default_model_id", "source_id", "prompt_id", "folder_id", "workflow_id", "extra_source_ids"):
                ...
                for col in ("chunks", "token_limit", "request_limit"):
                    if template.get(col) is not None:
                        create_kwargs[col] = template[col]
                for col in ("limited_token_mode", "limited_request_mode", "allow_system_prompt_override", "guardrails_enabled", "input_guardrails", "output_guardrails"):
                    if template.get(col) is not None:
                ...
                new_agent = AgentsRepository(conn).create(user, template.get("name") or "", "published", **create_kwargs)
...
class RemoveSharedAgent(Resource):
    @api.doc(params={"id": "ID of the shared agent"}, description="Remove a shared agent from the current user's shared list")
    def delete(self):
        decoded_token = request.decoded_token
        if not decoded_token:
            return make_response(jsonify({"success": False}), 401)
        ...
        try:
            with db_session() as conn:
                ...
                if looks_like_uuid(agent_id):
                    agent_row = conn.execute(
                        _sql_text("SELECT id FROM agents WHERE id = CAST(:id AS uuid) AND shared = true"),
                        {"id": agent_id},
                    ).fetchone()
                else:
                    agent_row = conn.execute(
                        _sql_text("SELECT id FROM agents WHERE legacy_mongo_id = :id AND shared = true"),
                        {"id": agent_id},
                    ).fetchone()
                if agent_row is None:
                    return make_response(jsonify({"success": False, "message": "Shared agent not found"}), 404)
                pg_agent_id = str(agent_row._mapping["id"])
