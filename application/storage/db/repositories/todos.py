...
class TodosRepository:
    ...
    def get(self, todo_id: str) -> Optional[dict]:
        result = self._conn.execute(
            text("SELECT * FROM todos WHERE id = CAST(:id AS uuid)"),
            {"id": todo_id},
        )
        row = result.fetchone()
    ...
    def update_title(self, todo_id: str, title: str) -> bool:
        result = self._conn.execute(
            text("UPDATE todos SET title = :title, updated_at = now() WHERE id = CAST(:id AS uuid)"),
            {"id": todo_id, "title": title},
        )
        return result.rowcount > 0

    def set_completed(self, todo_id: str, completed: bool = True) -> bool:
        result = self._conn.execute(
            text(
                "UPDATE todos SET completed = :completed, updated_at = now() WHERE id = CAST(:id AS uuid)"
            ),
            {"id": todo_id, "completed": completed},
        )
        return result.rowcount > 0

    def delete(self, todo_id: str) -> bool:
        result = self._conn.execute(
            text("DELETE FROM todos WHERE id = CAST(:id AS uuid)"),
            {"id": todo_id},
        )
        return result.rowcount > 0
