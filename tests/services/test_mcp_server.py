"""Tests for docsgpt/mcp_server.py.
...
class TestSearchDocsTool:
    ...
    async def test_missing_bearer_raises_permission_error(self):
        from docsgpt.mcp_server import search_docs

        with patch(
            "docsgpt.mcp_server.get_http_headers", return_value={}
        ):
    ...
    async def test_non_bearer_header_raises_permission_error(self):
        ...
        with patch(
            "docsgpt.mcp_server.get_http_headers",
    ...
    async def test_invalid_api_key_raises_permission_error(self):
        ...
        with (
            ...
            patch(
                "docsgpt.mcp_server.search", side_effect=InvalidAPIKey()
            ),
        ):
            with pytest.raises(PermissionError):
                await search_docs(query="hi")
    ...
    async def test_happy_path_passes_args_and_returns_hits(self):
        ...
        with (
            ...
            patch(
                "docsgpt.mcp_server.search", return_value=hits
            ) as mock_search,
        ):
    ...
    async def test_default_chunks_is_5(self):
        ...
        with (
            ...
            patch(
                "docsgpt.mcp_server.search", return_value=[]
            ) as mock_search,
        ):
