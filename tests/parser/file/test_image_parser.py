...
@patch("docsgpt.parser.file.image_parser.settings")
def test_image_parser_remote_true(mock_settings):
    ...
    with patch("docsgpt.parser.file.image_parser.requests.post", return_value=mock_response) as mock_post:
...
def test_image_parser_remote_false(mock_settings):
    ...
    with patch("docsgpt.parser.file.image_parser.requests.post") as mock_post:
        result = parser.parse_file(Path("img.png"))
