"""Tests for pure utility functions in docsgpt/worker.py.

These cover helpers that don't require a Celery runtime (no task
instantiation). Aimed at maximizing coverage of ``docsgpt/worker.py``
...
class TestMetadataFromFilename:
    def test_returns_title_dict(self):
        from docsgpt.worker import metadata_from_filename
        assert metadata_from_filename("doc.txt") == {"title": "doc.txt"}


class TestNormalizeFileNameMap:
    def test_empty_returns_empty_dict(self):
        from docsgpt.worker import _normalize_file_name_map
...
class TestGetDisplayName:
    def test_returns_none_for_empty_inputs(self):
        from docsgpt.worker import _get_display_name
...
class TestApplyDisplayNames:
    def test_non_dict_structure_returned_as_is(self):
        from docsgpt.worker import _apply_display_names_to_structure
...
class TestGenerateRandomString:
    def test_length(self):
        from docsgpt.worker import generate_random_string
...
class TestIsPathSafe:
    def test_allows_file_under_base(self, tmp_path):
        from docsgpt.worker import _is_path_safe
        base = str(tmp_path)
...
class TestValidateZipSafety:
    ...
    def test_accepts_valid_small_zip(self, tmp_path):
        from docsgpt.worker import _validate_zip_safety
    ...
    def test_rejects_too_many_files(self, tmp_path):
        ...
        with patch("docsgpt.worker.MAX_FILE_COUNT", 5):
            with pytest.raises(ZipExtractionError):
                _validate_zip_safety(str(zip_path), str(tmp_path))
...
class TestExtractZipRecursive:
    def test_extracts_flat_zip(self, tmp_path):
        from docsgpt.worker import extract_zip_recursive
    ...
    def test_nested_archives_share_one_expansion_budget(self, tmp_path):
        from docsgpt.worker import extract_zip_recursive, ZipExtractionError
        ...
        extract_to = tmp_path / "out"
        extract_to.mkdir()
        with patch("docsgpt.worker.MAX_UNCOMPRESSED_SIZE", limit), patch(
            "docsgpt.worker.MAX_COMPRESSION_RATIO", 10_000,
        ):
            with pytest.raises(ZipExtractionError):
                extract_zip_recursive(str(outer), str(extract_to))
...
class TestDownloadFile:
    def test_writes_file_on_success(self, tmp_path):
        from docsgpt.worker import download_file
        ...
        with patch(
            "docsgpt.worker.requests.get", return_value=mock_response,
        ):
...
class TestUploadIndex:
    def test_non_faiss_posts_data_only(self, tmp_path):
        ...
        with patch(
            ...
            "docsgpt.worker.settings.INTERNAL_KEY", "k"
        ), patch(
            "docsgpt.worker.requests.post", return_value=mock_response,
        ) as mock_post:
    ...
    def test_faiss_missing_file_raises(self, tmp_path):
        ...
        with patch(
            "docsgpt.worker.settings.VECTOR_STORE", "faiss"
        ):
    ...
    def test_faiss_uploads_both_files(self, tmp_path):
        ...
        with patch(
            ...
            "docsgpt.worker.settings.INTERNAL_KEY", ""
        ), patch(
            "docsgpt.worker.requests.post", return_value=mock_response,
        ) as mock_post:
            upload_index(str(tmp_path), {"source_id": "1"})
...
class TestCountStructureFiles:
    ...
    def test_counts_nested_leaves_not_top_level_keys(self):
        from docsgpt.worker import count_structure_files
...
class TestDirectoryStructureAccumulatesChunks:
    ...
    def test_multi_chunk_file_sums_its_chunks(self):
        from docsgpt.worker import add_file_to_structure, count_structure_files
    ...
    def test_empty_path_is_ignored(self):
        from docsgpt.worker import add_file_to_structure
