...
**Upgrading:** earlier images always included tesseract. If your deployment
runs `OCR_ENABLED=true` with the `tesseract` engine, add
`INSTALL_TESSERACT=true` to `.env` before rebuilding, or OCR of scanned pages
stops working after the rebuild.
</Callout>
