...
All outgoing HTTP requests from API-based tools (including API Tool, Telegram Tool, Brave Tool, Read Webpage Tool, etc.) can be routed through a proxy.

Set the proxy by either:
- Passing a `proxy` key in the tool config, or
- Setting the `API_TOOL_PROXY` environment variable:

    export API_TOOL_PROXY="http://myproxy.example.com:8080"

If not set, requests will be made directly.
