...
class BaseAnswerResource:
    ...
    def complete_stream(
        ...
        **kwargs: Any,
        ...
        """Stream the conversation response and record its execution trace.

        Accepts every argument of :meth:`_complete_stream_body`. The trace is
        activated for the whole stream -- which runs in the SSE pump thread,
        not the request thread -- and written exactly once when the stream
        ends, however it ends.

        Args:
            request_id: The request's id, generated by the route before the
                agent was built so pre-fetch retrieval shares it. A
                continuation's saved id takes precedence; absent both, a new
                one is minted.
            trace: The trace the route started (it already holds the
                pre-fetch retrieval). A new one is started when omitted.

        Yields:
            Server-sent event strings.
        """
        bound = inspect.signature(self._complete_stream_body).bind(*args, **kwargs)
        continuation = bound.arguments.get("_continuation")
        request_id = (
            continuation.get("request_id") if continuation else None
        ) or request_id or str(uuid.uuid4())
        decoded_token = bound.arguments.get("decoded_token") or {}
        ...
        with tracing.activate(trace):
            try:
                yield from self._complete_stream_body(
                    *bound.args, request_id=request_id, **bound.kwargs
                )
            finally:
                tracing.flush(trace)

    def _complete_stream_body(
        self,
        question: str,
        agent: Any,
