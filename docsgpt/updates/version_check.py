...
Fired on every Celery worker boot (see ``docsgpt/celery_init.py``
``worker_ready`` handler) and on a 7h periodic schedule (see the
``version-check`` entry in ``docsgpt/api/user/tasks.py``). Posts
...
from docsgpt.version import get_version
...
def _fetch(instance_id: str) -> Optional[Dict[str, Any]]:
    ...
    if version in ("", "unknown"):
        ...
        logger.warning(
            ...
            "Set __version__ in docsgpt/version.py to a valid "
            "version string.",
            version,
        )
        return None
