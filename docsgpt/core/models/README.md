...
matching provider plugin under `docsgpt/llm/providers/`.
...
file to `docsgpt/llm/providers/<name>.py`:

```python
from docsgpt.llm.providers.base import Provider
from docsgpt.llm.my_provider import MyLLM
...
Register it in `docsgpt/llm/providers/__init__.py` (one line in
...
catalog under `docsgpt/core/models/`. Operators use this to:
...
- Add a brand-new non-OpenAI provider — that needs a Python plugin
  under `docsgpt/llm/providers/` (see "Add a provider with its own
