...
The seeder is invoked by the ``python -m docsgpt.seed.commands init``
...
class DatabaseSeeder:
    ...
    def _coerce_uuid_fk(raw) -> Optional[str]:
        ...
        Mirrors the route-side handling in ``docsgpt/api/user/agents/routes.py``:
    ...
    def initialize_from_env(cls, worker=None):
        ...
        (``docsgpt.storage.db.engine``), so no explicit wiring is
        required here.
        """
        return cls()
