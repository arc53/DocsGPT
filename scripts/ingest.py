...
def ingest(yes: bool = typer.Option(False, "-y", "--yes", prompt=False,
           ...
           s3_bucket: Optional[str] = typer.Option(None, help="Name of the S3 bucket to use as document store"),
           s3_documents_folder: Optional[str] = typer.Option(None, "--s3-doc-folder", 
                                                            help="""Path of the S3 folder from which to download documents
                                                                E.g. --s3-folder docsgpt/documents"""),
           s3_save_folder: Optional[str] = typer.Option(None, help="Path of the S3 folder in which to save index files"),
           s3_assume_role: Optional[bool] = typer.Option(False, "--s3-assume", help="Whether to assume an IAM role"),
           aws_assume_role_profile: Optional[str] = typer.Option(None, "--s3-assume-profile", help="AWS config profile to use for role assumption.")
           ):
    """
    ...
    def process_one_docs(directory, folder_name):
        ...
        if len(sys.argv) > 1 and yes:
            ...
            get_user_permission(docs, folder_name, use_s3, s3_bucket, s3_save_folder, s3_assume_role, aws_assume_role_profile)

    if use_s3:
        download_from_s3(s3_bucket, s3_documents_folder, s3_assume_role, aws_assume_role_profile)
...
def download_from_s3(s3_bucket=None, s3_prefix=None, s3_assume_role=False, aws_assume_role_profile=None):
    # Set defaults from environment variables if not provided via command line
    s3_bucket = s3_bucket or os.environ.get("S3_BUCKET")
    s3_prefix= s3_prefix or os.environ.get("S3_DOCUMENTS_FOLDER")
    ...
    if not s3_prefix:
        print(
            ...
            f"All files in S3 bucket '{s3_bucket}' will be downloaded. "
    ...
    try:
        s3_docs_count = sum(
            1
            for obj in s3.Bucket(s3_bucket).objects.filter(Prefix=s3_prefix)
        ...
        if str(e.response["Error"]["Code"]) == "AccessDenied":
            print(
                f"You do not have AWS permissions to access the specified bucket,'{s3_bucket}'. "
    ...
    if s3_prefix == "" and s3_docs_count < 1:
        raise ValueError(f"No objects were found in {s3_bucket} bucket")
    if s3_docs_count < 1:
        raise ValueError(
            f"No objects were found within the '{s3_prefix}' folder in {s3_bucket} bucket"
        )
